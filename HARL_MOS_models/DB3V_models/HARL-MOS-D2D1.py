...
song_birds = [
    "Agelaius phoeniceus", "Cardinalis cardinalis", "Setophaga aestiva",
    "Setophaga ruticilla", "Spinus tristis", "Turdus migratorius"
]
call_birds = [
    "Tringa semipalmata", "Certhia americana", "Corvus brachyrhynchos",
    "Molothrus ater"
]
# 路径定义
...
def load_data(region_paths, bird_classes):
    ...
    test_dataset_region1 = datasets['region1']
    ...
    data_loaders['test_region1'] = DataLoader(test_dataset_region1, batch_size=32, shuffle=False, drop_last=False)

    return data_loaders


# 训练流程
def train_model(model, criterion_main, criterion_aux, optimizer, train_loader, val_loader1, val_loader2, num_epochs=50, save_epoch=None):
    ...
    for epoch in range(num_epochs):
        ...
        val_acc2 = evaluate_on_loader(val_loader2)
        avg_val_acc = (val_acc1 + val_acc2) / 2

        print(f'           >>> Val Acc1: {val_acc1:.4f} | Val Acc2: {val_acc2:.4f} | Avg: {avg_val_acc:.4f}')

        # 保存验证集准确率平均最高的模型
        if val_acc1 > best_avg_val_acc_1:
            if val_acc2 > best_avg_val_acc_2:
                best_avg_val_acc_1 = val_acc1
                best_avg_val_acc_2 = val_acc2
                best_model_wts = model.state_dict()
                torch.save(best_model_wts, "Resnet_GM_Mixstyle_d2_best_avg_val_renew_lr_1e_8h_0.4.pth")
                print(f"New best model saved at epoch {epoch + 1} with Avg Val Acc: {avg_val_acc:.4f}")
...
# bird_classes = [...]  # 填入实际的鸟类类别列表
    ...
    model = train_model(model, criterion_main, criterion_aux, optimizer, data_loaders['train'],
                        data_loaders['test_region3'], data_loaders['test_region1'], num_epochs=50, save_epoch=50)
    model.eval()
    model.load_state_dict(
        torch.load("Resnet_GM_Mixstyle_d2_best_avg_val_renew_lr_1e_8h_0.4.pth"))
