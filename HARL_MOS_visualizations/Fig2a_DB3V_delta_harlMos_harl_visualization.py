...
# 1. 读取 Excel 文件
file_path = "CompHARL-MOS-with-HARLF1.xlsx"
excel_file = pd.ExcelFile(file_path)
df_raw = excel_file.parse(sheet_name=0, skiprows=2)

# 2. 提取 HARL-MOS 数据
df_harlmos = df_raw.iloc[1:7, 2:].copy()
df_harlmos.columns = df_raw.iloc[0, 2:].tolist()
df_harlmos.insert(0, "Domain", df_raw.iloc[1:7, 1].tolist())
df_harlmos = df_harlmos.dropna(axis=1, how="all")
df_harlmos.iloc[:, 1:] = df_harlmos.iloc[:, 1:].apply(pd.to_numeric, errors='coerce')

# 3. 提取 HARL 数据
df_harl = df_raw.iloc[9:15, 2:].copy()
df_harl.columns = df_raw.iloc[8, 2:].tolist()
df_harl.insert(0, "Domain", df_raw.iloc[9:15, 1].tolist())
